...
public class FeaturesController {
	 ...
	 * @return
	 */
	@PostMapping(path = "/allByApp")
	public @ResponseBody Iterable<FeatureVO> getActiveByApp(@RequestHeader("Authorization") String idToken, @RequestBody FeatureVO tFeature) {

		List<FeatureVO> allFeatures = new ArrayList<>();
		logger.info("appCode received: {}", tFeature.getAppName());
		String uid = FirebaseUtils.getDecodedFireBaseToken(idToken);
		if (uid != null || shouldSkipAuthChecks()) {
			// This returns a JSON or XML with the items
			List<Feature> tFeatures = featureRepository.findActiveFeaturesByAppName(tFeature.getAppName());
			for (Feature eachFeature : tFeatures) {
				allFeatures.add(FeatureConverter.convertEntityToVO(eachFeature));
			}
			return allFeatures;
